...
# ─────────────────────────────────────────────────────────────────────────────
# Foto via Wikimedia Commons (API)
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_data(show_spinner=False, ttl=3600)
def get_wikimedia_photo(name: str) -> str | None:
    """Tenta buscar uma foto do candidato na API do Wikimedia Commons.
    Retorna a URL da imagem ou None se não encontrar."""
    try:
        search_name = str(name).title()
        # 1. Busca no Wikipedia pt para encontrar o título da página
        resp = requests.get(
...
def get_candidate_description(name: str, cargo: str, uf: str, partido: str) -> str:
    """Gera uma breve descrição do candidato buscando no Wikipedia pt."""
    try:
        search_name = str(name).title()
        resp = requests.get(
            "https://pt.wikipedia.org/w/api.php",
...
def show_candidate_details(df: pd.DataFrame, uf: str, candidate_name: str):
    """Flashcard completo com foto, descrição e gráfico de votos por zona."""
    ...
    # Foto e descrição (buscados em paralelo via cache)
    ...
    if photo_url:
        photo_html = f'<img class="detail-card-photo" src="{photo_url}" alt="{name_title}">'
    else:
        photo_html = '<div class="detail-card-photo-placeholder">👤</div>'

    wiki_name = name_title.replace(" ", "_")
    ...
    with c4:
        metric_card("Ocupação", str(row.get("Ocupação", "-")).title(), "#f4a261")

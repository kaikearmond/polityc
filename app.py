...
def make_brazil_map(summary: pd.DataFrame, selected_uf: str | None = None):
    ...
    if geojson:
        ...
        fig.add_trace(go.Choropleth(
            geojson=geojson,
            featureidkey="properties.sigla",  # ex: "SP", "AC"
            locations=summary["UF"],
            z=summary["num_eleitos"],
            colorscale="Blues",
            customdata=summary[["UF", "state_name", "num_eleitos", "num_zonas"]].values,
            hovertemplate=(
                "<b>%{customdata[1]}</b><br>"
            ...
            marker_line_color="rgba(120,130,160,0.5)",
        ...
        geo = dict(
            ...
            showland=False,       # sem terra de outros países
            showocean=False,      # sem oceano visível
            showcountries=False,  # sem fronteiras de outros países
            showlakes=False,
            bgcolor="rgba(13,17,23,1)",
            fitbounds="locations",   # zoom automático para o Brasil
        ...
        fig.add_trace(go.Choropleth(
            ...
            colorbar=dict(title=dict(text="Eleitos", font=dict(color="white")), tickfont=dict(color="white")),
        ))
        if selected_uf:
            fig.add_trace(go.Choropleth(
